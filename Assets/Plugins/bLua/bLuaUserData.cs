...
namespace bLua
{
    ...
    public sealed class bLuaHiddenAttribute : Attribute
    {
    }

    [AttributeUsage(AttributeTargets.Class)]
    public sealed class bLuaUserDataAttribute : Attribute
    {
    }
    ...
    public static class bLuaUserData
    {
        ...
        public static void PushReturnTypeOntoStack(bLuaInstance _instance, MethodCallInfo.ParamType _returnType, object _result)
        {
            switch (_returnType)
            {
                ...
                case MethodCallInfo.ParamType.Bool:
                    {
                        Lua.PushObjectOntoStack(_instance, (bool)_result);
                        return;
                    }
                case MethodCallInfo.ParamType.Int:
                    {
                        Lua.PushObjectOntoStack(_instance, (int)_result);
                        return;
                    }
                case MethodCallInfo.ParamType.Float:
                    {
                        Lua.PushObjectOntoStack(_instance, (float)_result);
                        return;
                    }
                case MethodCallInfo.ParamType.Double:
                    {
                        Lua.PushObjectOntoStack(_instance, (double)_result);
                        return;
                    }
                case MethodCallInfo.ParamType.Str:
                    {
                        Lua.PushObjectOntoStack(_instance, (string)_result);
                        return;
                    }
        ...
        public static void DebugInfoReport(bLuaInstance _instance)
        {
            ...
            foreach (var p in _instance.s_liveObjects)
            {
                if (p == null)
                {
                    continue;
